...
function OverviewTab({ evaluations, teachers, teachingAssignments }: { evaluations: EvaluationRow[], teachers: UserItem[], teachingAssignments: any[] }) {
  ...
  const handleViewReport = (evaluationId: string) => {
    navigate(`/cerapan/results/${evaluationId}`);
  };
                  ...
                  <TableCell align="center">
                    <Box sx={{ display: 'flex', gap: 1, justifyContent: 'center', flexWrap: 'wrap' }}>
                      ...
                      {(row.obs1Status === 'pending') && row.id && (
                        ...
                        >
                          {(!row.scheduledDate || !row.scheduledTime) ? 'Belum Dijadualkan' : 'Mula Cerapan 1'}
                        </Button>
                      )}
                      {row.obs2Status === 'pending' && title.includes('Cerapan 2') && row.obs1Status === 'submitted' && row.id && (
                        <Button
                          ...
                          disabled={!row.scheduledDate || !row.scheduledTime}
                        >
                          {(!row.scheduledDate || !row.scheduledTime) ? 'Belum Dijadualkan' : 'Mula Cerapan 2'}
                        </Button>
                      )}
                      ...
                      {row.id && (
                        <Button size="small" startIcon={<Eye size={16} />} onClick={() => navigate(`/cerapan/results/${row.id}`)}>
                          Lihat
                        </Button>
                      )}
                    </Box>
                ...
                <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
                  {(() => {
                    ...
                    const filteredRows = allRows.filter(row => {
                      ...
                      if (scheduleFilter === 1) {
                        // Ready for Obs 1: obs1 is pending
