...
export class AttendanceController {
  ...
  async getAttendanceHistory(
    ...
    const records = await this.attendanceService.getRecordsByRange(
      userId,
      startDate,
      endDate,
    );
  ...
  @Put('reason')
  async updateReason(
    @Body() body: { recordId: string; type: 'in' | 'out'; reason: string },
  ) {
    if (!body.recordId || !body.type || body.reason === undefined) {
      throw new BadRequestException(
        'Record ID, type (in/out) and reason are required',
      );
    }
    return this.attendanceService.updateReason(
      body.recordId,
      body.type,
      body.reason,
    );
